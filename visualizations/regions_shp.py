...
for var in ds.variables:
    print(var, ds.variables[var].shape, ds.variables[var].dimensions)
...
# check area_km2 — how sparse is it?
area = ds['area_km2'].values  # shape (50, 360, 720)
print('non-NaN cells per timestep:', np.sum(~np.isnan(area[0])))
print('lat range:', ds['lat'].values.min(), ds['lat'].values.max())
print('lon range:', ds['lon'].values.min(), ds['lon'].values.max())
...
def interactive_global_grid_pctloss():
        ...
        return go.Frame(
            ...
            data=[go.Scattergeo(
                ...
                marker=dict(
                    size=4,
                    color=pct[valid],
                    colorscale=[[0,'#042C53'],[0.5,'#378ADD'],[1,'#d0e8f5']],
                    cmin=min_val,
                    cmax=0,
                    colorbar=dict(
    ...
    layout = go.Layout(
        geo=dict(projection_type='natural earth', showland=True, landcolor='#e8e6df',
                 showocean=True, oceancolor='#d0e8f5', showcoastlines=True,
                 coastlinecolor='#aaa', showframe=False, bgcolor='rgba(0,0,0,0)'),
