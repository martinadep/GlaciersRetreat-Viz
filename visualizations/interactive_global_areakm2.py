...
def interactive_global_areakm2():   # <-- aggiungi questa riga
    ...
    regions = ['ACN','ACS','ALA','ANT','ASC','ASE','ASN','ASW','CAU','CEU',
               'GRL','ISL','NZL','RUA','SA1','SA2','SCA','SJM','TRP','WNA']
    ...
    slider_steps = [
        dict(args=[[str(y)], dict(frame=dict(duration=300, redraw=True), mode='immediate')],
    ...
    layout = go.Layout(
        ...
        sliders=[dict(active=len(years)-1, steps=slider_steps, x=0.05, len=0.9, y=-0.05,
